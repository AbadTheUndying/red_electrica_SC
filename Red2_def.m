...
function mismatch = pf_mismatch(delta_free, Pi_vec, Kmat, slack, N)
    ...
    delta = zeros(N,1);
    delta(non_slack) = delta_free; %vector desfases costruido 
...
% Aristas propuestas
sEdge = [1 1 2 3 4 5 6 7 8 5 6 9 9 9 10 11 11 13 14 12 13 13 16 7 18 18 15 14 17]; %nodos iniciales

tEdge = [2 3 9 4 5 6 7 8 19 10 10 10 11 13 14 12 13 14 15 16 16 17 17 18 19 20 20 20 20];%nodos destino
...
% Pi en GW
%Potencia neta media que sale o entra de un nodo
Pi = [
   0.623
   0.265
  -0.093
...
%Ahora vamos a construir el vector de inercia Mstar
...
%Matrix de K(factor de cesion de potencia)
%  MATRIZ DE ACOPLAMIENTO K
...
for e = 1:Nedges
    ii = sEdge(e);
    jj = tEdge(e);
    d_geo(e) = haversine_km(lat(ii), lon(ii), lat(jj), lon(jj));
end

% 2) Factor de correccion del trazado real alpha_ij
%    1.10 = corredor relativamente directo
%    1.15 = corredor normal
%    1.20 = corredor mas sinuoso / relieve
%    1.30 = corredor montanoso fuerte (Pirineos)
alpha = [ ...
    1.15
    1.20
    1.20
...
% 3) Entre dos zonas no siempre hay un solo corredor equivalente, numero de circuitos equivalentes n_ij
%    2 en corredores troncales fuertes de la red reducida
%    1 en corredores normales
nij = [ ...
    2
    1
    1
...
% 4) Kedge en GW
%    Formula en MW: K = n * V^2 / (xkm * L)
%    Como V esta en kV y x en ohm/km, sale MW
%    Se divide entre 1000 para pasar a GW
...
opts_fs = optimoptions('fsolve', ...
    'Display','iter', ...
    'TolFun',1e-10, ...
    'TolX',1e-10, ...
    'MaxIter',2000, ...
    'Algorithm','levenberg-marquardt'); % definicion de opciones para fsolve
...
dth = delta_op - delta_op.';
Pcalc = sum(K .* sin(dth), 2);
resid = Pcalc - Pi;
...
% Escenario B: salida total del nodo 11
% Pi_fault = Pi;
% Pi_fault(11) = 0;
% K_fault = K;
% K_fault(11,:) = 0;
...
[t1, X1] = ode15s(@(tt,x) swing_eq(tt, x, Pi,       K,      Mi, Di, N), ...
                  [0 t_fault], x0, ode_opts);

[t2, X2] = ode15s(@(tt,x) swing_eq(tt, x, Pi_fault, K_fault, Mi, Di, N), ...
                  [t_fault t_end], X1(end,:)', ode_opts);
...
for k = 1:length(tSim)
    for e = 1:Ne
        ii = sEdge(e);
        jj = tEdge(e);
        Fline(k,e) = K(ii,jj) * sin(theta(k,ii) - theta(k,jj));
    end
end
...
grid on;
xlabel('t (s)');
ylabel('\theta_i (grados)');
title('Angulos nodales');
...
grid on;
xlabel('t (s)');
ylabel('Flujo por linea (GW)');
title('Flujos de linea');

%% ===== VOLTAJES AC RECONSTRUIDOS MEJORADOS =====
% Visualizacion mas util: referidos al slack y diferencia respecto al slack

nodesPlot = [11 14 15];   % B11, B14, B15
slack = 14;
Vamp = 1.0;               % amplitud en p.u.

% Ventana corta alrededor del fallo
tStart = 1.95;
tStop  = 2.10;

% Malla fina para ver bien 50 Hz
tFine = linspace(tStart, tStop, 5000);
...
% Voltajes absolutos reconstruidos
...
% Voltajes referidos al slack (quitando fase comun)
vRel = zeros(length(tFine), length(nodesPlot));
for m = 1:length(nodesPlot)
    ii = nodesPlot(m);
    thetaRel = thetaFine(:,ii) - thetaFine(:,slack);
    vRel(:,m) = Vamp * cos(w0*tFine(:) + thetaRel);
end

% Diferencia respecto al slack
...
subplot(3,1,1)
plot(tFine, vAbs, 'LineWidth', 1.1);
xline(t_fault,'k--','LineWidth',1.4);
grid on;
xlabel('t (s)');
ylabel('v_i(t) [p.u.]');
title('Voltajes AC absolutos reconstruidos');
legend(nodeNames(nodesPlot), 'Interpreter','none', 'Location','best');

subplot(3,1,2)
plot(tFine, vRel, 'LineWidth', 1.1);
xline(t_fault,'k--','LineWidth',1.4);
grid on;
xlabel('t (s)');
ylabel('v_i^{rel}(t) [p.u.]');
title('Voltajes reconstruidos referidos al slack');

subplot(3,1,3)
plot(tFine, dv, 'LineWidth', 1.1);
xline(t_fault,'k--','LineWidth',1.4);
grid on;
xlabel('t (s)');
ylabel('\Deltav_i = v_i - v_{slack}');
title('Diferencia de voltaje respecto al slack');

%% ===== ANGULOS RELATIVOS AL SLACK =====
theta_rel = theta - theta(:,slack);

figure('Name','Angulos relativos al slack','NumberTitle','off');
plot(tSim, rad2deg(theta_rel), 'LineWidth', 1.2);
xline(t_fault,'k--','LineWidth',1.4);
grid on;
xlabel('t (s)');
ylabel('\theta_i - \theta_{slack} [grados]');
title('Angulos relativos al slack');
legend(nodeNames, 'Interpreter','none', 'Location','eastoutside');
%% ===== TABLA DE ARISTAS =====
